...
1. Setup Box for use with curl if you have not done so already
   ...
   3. Edit `~/.netrc` such that first first line is `machine ftp.box.com`, the second line is `login <your_smu_email_address`, and the third line is `password <your_unique_box_password_set_in_the_previous_step>`
